...
export default function NavArtItem({ data, handlePageSelect, selectedPage }) {
  ...
  return(
    <>
      <li className="flex items-center cursor-pointer hover:text-sky-500 my-2" >
        <span onClick={e => handlePageSelect(e)} className={selectedPage === navTitle || homepage && !selectedPage ? "text-orange-500" : ""}>
          <Link href={`/${slug}`} >
            {navTitle}
          </Link>
        </span>
      ...
      <ul className="years-list" style={{display: showYears ? 'block' : 'none'}}>
        {years?.map(year => (
          ...
          <li key={year + slug} className="hover:text-sky-500">
            <span onClick={e => handlePageSelect(e)} className={selectedPage === year ? "text-orange-500" : ""}>
              <Link 
                href={`/${slug}?year=${year}`}
